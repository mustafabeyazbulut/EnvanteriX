...
{
    public class ActiveModelCommandHandler : BaseHandler, IRequestHandler<ActiveModelCommand, Unit>
    {
        private readonly ModelRules _modelRules;
        public ActiveModelCommandHandler(IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, ModelRules modelRules)
            : base(mapper, unitOfWork, httpContextAccessor)
        {
            _modelRules = modelRules;
        }

        public async Task<Unit> Handle(ActiveModelCommand request, CancellationToken cancellationToken)
        {
