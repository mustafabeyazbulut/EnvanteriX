...
{
    public class ActivePortal365CommandHandler : BaseHandler, IRequestHandler<ActivePortal365Command, Unit>
    {
        private readonly Portal365Rules _portal365Rules;
        public ActivePortal365CommandHandler(IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, Portal365Rules portal365Rules) : base(mapper, unitOfWork, httpContextAccessor)
        {
            _portal365Rules = portal365Rules;
        }

        public async Task<Unit> Handle(ActivePortal365Command request, CancellationToken cancellationToken)
        {
