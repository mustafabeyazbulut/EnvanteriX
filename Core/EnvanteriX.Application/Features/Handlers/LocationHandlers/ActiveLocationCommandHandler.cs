...
{
    public class ActiveLocationCommandHandler : BaseHandler, IRequestHandler<ActiveLocationCommand, Unit>
    {
        private readonly LocationRules _locationRules;
        public ActiveLocationCommandHandler(IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, LocationRules locationRules)
            : base(mapper, unitOfWork, httpContextAccessor)
        {
            _locationRules = locationRules;
        }
        public async Task<Unit> Handle(ActiveLocationCommand request, CancellationToken cancellationToken)
        {
