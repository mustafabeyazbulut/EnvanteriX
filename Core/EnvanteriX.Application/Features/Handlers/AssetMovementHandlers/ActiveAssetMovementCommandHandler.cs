...
{
    public class ActiveAssetMovementCommandHandler : BaseHandler, IRequestHandler<ActiveAssetMovementCommand, Unit>
    {
        private readonly AssetMovementRules _assetMovementRules;
        public ActiveAssetMovementCommandHandler(IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, AssetMovementRules assetMovementRules) : base(mapper, unitOfWork, httpContextAccessor)
        {
            _assetMovementRules = assetMovementRules;
        }

        public async Task<Unit> Handle(ActiveAssetMovementCommand request, CancellationToken cancellationToken)
        {
