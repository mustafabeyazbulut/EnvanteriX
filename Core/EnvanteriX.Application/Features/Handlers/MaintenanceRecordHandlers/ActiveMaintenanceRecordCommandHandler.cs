...
{
    public class ActiveMaintenanceRecordCommandHandler : BaseHandler, IRequestHandler<ActiveMaintenanceRecordCommand, Unit>
    {
        private readonly MaintenanceRecordRules _maintenanceRecordRules;
        public ActiveMaintenanceRecordCommandHandler(IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, MaintenanceRecordRules maintenanceRecordRules)
            : base(mapper, unitOfWork, httpContextAccessor)
        {
            _maintenanceRecordRules = maintenanceRecordRules;
        }

        public async Task<Unit> Handle(ActiveMaintenanceRecordCommand request, CancellationToken cancellationToken)
        {
