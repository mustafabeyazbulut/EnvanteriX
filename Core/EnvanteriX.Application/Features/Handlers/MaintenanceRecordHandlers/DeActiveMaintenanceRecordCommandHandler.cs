...
{
    public class DeActiveMaintenanceRecordCommandHandler : BaseHandler, IRequestHandler<DeActiveMaintenanceRecordCommand, Unit>
    {
        private readonly MaintenanceRecordRules _maintenanceRecordRules;
        public DeActiveMaintenanceRecordCommandHandler(IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, MaintenanceRecordRules maintenanceRecordRules)
            : base(mapper, unitOfWork, httpContextAccessor)
        {
            _maintenanceRecordRules = maintenanceRecordRules;
        }

        public async Task<Unit> Handle(DeActiveMaintenanceRecordCommand request, CancellationToken cancellationToken)
        {
